...
\begin{abstract}
Data space adoption in public-sector environments requires more than deploying a standards-compliant connector runtime. Organizations also need repeatable deployment artifacts, operator-facing publication workflows, controlled access to local and remote assets, traceable transfer handling, and integration paths with the systems where data is curated or consumed. This paper presents TOPIC Connector, a municipal-oriented software toolkit built around Eclipse Dataspace Components (EDC). TOPIC Connector packages an EDC runtime with a management user interface, local asset ingestion, download-capture support, PostgreSQL persistence, Nginx gateway routing, Docker Compose deployment profiles, and ArcGIS-oriented integration hooks. The software is released as an Apache-2.0-licensed artifact with a documented local reproduction path and validation scripts. The paper describes the software architecture, explains the engineering methodology embodied by the toolkit, and reports service-level validation checks for stack health, local asset upload, and download-capture registration. The results do not claim a complete interoperability benchmark or a mature production certification; rather, they show that TOPIC Connector provides a reusable and executable baseline for early-stage municipal connector deployment and operation.
\end{abstract}

\begin{IEEEkeywords}
data spaces, Eclipse EDC, data connector, municipal interoperability, research software, asset publication
\end{IEEEkeywords}
...
\section{Software Artifact and Availability}
...
Version described & \texttt{v1.0.1} \\
...
Reproduction path & \texttt{docker-compose.yaml}, \texttt{.env.example}, \texttt{examples/}, and \texttt{scripts/validate\_*.sh} \\
...
The software release includes a \texttt{CITATION.cff} file, a \texttt{NOTICE} file, a changelog, contribution and security notes, and a reproducibility guide. A permanent archival DOI can be associated with the release through Zenodo or an equivalent archive; the executable software artifact itself is already identified by the public release tag used in this manuscript.
